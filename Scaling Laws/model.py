...
class LinearModel(nn.Module):
    def __init__(self, model_size):
        super().__init__()
        self.fc1 = nn.Linear(784, int(4.6 * model_size))
        self.fc2 = nn.Linear(int(4.6 * model_size), int(2.3 * model_size))
        self.fc3 = nn.Linear(int(2.3 * model_size), 10)
        self.dropout = nn.Dropout(0.2)
        self.batch_norm1 = nn.BatchNorm1d(int(4.6 * model_size))
        self.batch_norm2 = nn.BatchNorm1d(int(2.3 * model_size))
