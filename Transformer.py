...
def run_epoch(data_iter, model, loss_compute, optimizer, scheduler,
    ...
    for i, batch in enumerate(data_iter):
        ...
        greedy_decode(model, batch.src, batch.src_mask, 71, 0)
        ...
        if i % 4 == 1 and (mode == "train" or mode == "train+log"):
...
def train_worker(
    ...
    for epoch in range(config["num_epochs"]):
        ...
        model.train()
        print(f"[GPU{device}] Epoch {epoch} Training ====", flush=True)
        ...
        print(f"[GPU{device}] Epoch {epoch} Validation ====", flush=True)
        model.eval()
        sloss = run_epoch(
...
def train_model(vocab_src, vocab_tgt, spacy_de, spacy_en, config):
    if config["distributed"]:
        ...
        train_worker(
            'cpu', 1, vocab_src, vocab_tgt, spacy_de, spacy_en, config, False
        )
